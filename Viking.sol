...
abstract contract Context {
 ...
 library Strings {
  ...
  contract ERC721A is
    ...
    uint256 public JarlsLockingTime = 7 days;

    uint256 public KarlsLockingTime = 14 days;
 ...
 library SafeMath {
  ...
  contract THE_VIKINGS is ERC721A,Ownable{
    ...
    {
        require(Royal_Sale == true, "RoyalSale_NOT_STARTED");
        require(!Karls_Sale, "only Karls are allowed to mint");
        require(!Jarls_Sale, "only Jarls are allowed to mint");
        require(totalMintedInFirstSale + _count <= firstSaleMintingLimit,"Max Limit Reached");
        require(userMintedNFT[msg.sender] + _count <= 2,"USER_MAX_LIMIT_REACHED");
    ...
    {
        require(Jarls_Sale == true, "JarlsSale_NOT_STARTED");
        require(!Royal_Sale, "only Royals are allowed to mint");
        require(!Karls_Sale, "only Karls are allowed to mint");
    ...
    //  3rd sale
    function KarlsVikingsSale(uint256 _count) public
    {
        require(Karls_Sale == true, "KarlsSale_NOT_STARTED");
        require(!Royal_Sale, "only Royals are allowed to mint");
        require(!Jarls_Sale, "only Jarls are allowed to mint");
    ...
    // * Admin Fuctions * //
    // * Set Sale * //
    function RoyalSALE() public onlyOwner {
    if(Royal_Sale == true){
        Royal_Sale = false;
        }
        else{
    ...
    function JarlsSALE() public onlyOwner{
        if(Jarls_Sale == true){
        Jarls_Sale = false;
        }
        else{
    ...
    function KarlsSALE() public onlyOwner{
        if(Karls_Sale == true){
        Karls_Sale = false;
        }
        else{
    ...
    function setJarlsLockingTime(uint256 _time) public onlyOwner{
      JarlsLockingTime = _time;
    }

    function setKarlsLockingTime(uint256 _time) public onlyOwner{
      KarlsLockingTime = _time;
    }
